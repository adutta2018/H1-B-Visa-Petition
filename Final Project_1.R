...
mydata %>%
  ...
  group_by(JOB_TITLE) -> jobwage

wage <- ggplot(jobwage, aes(x=reorder(JOB_TITLE,PREVAILING_WAGE,median),y=PREVAILING_WAGE)) +
...
mydata%>%
mutate(EMPLOYER_NAME = as.character(EMPLOYER_NAME)) -> mydata
...
g <- ggplot(job_filter(mydata,job_list), aes(x=JOB_INPUT_CLASS,y= PREVAILING_WAGE)) +
...
g <- map_gen(job_filter(mydata,job_list),"TotalApps",USA,Ntop = 3)
